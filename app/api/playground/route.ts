...
export async function POST(request: Request) {
  try {
    const {
      messages,
      model,
      // Optional image generation options sent from client UI
      imageOptions,
      // Force the image generation tool call (responses API) regardless of heuristic
      tool_choice,
      // Back-compat: simple boolean toggle from UI to force image generation
      forceImageTool,
      // Optional vision analysis request
      analyze,
      ...
      tool_choice?: { type: string } | "auto" | "none" | "required"
      forceImageTool?: boolean
      analyze?: { imageUrl: string; detail?: "low" | "high" | "auto" }
    }
    ...
    // If an image analysis request is provided, use Responses API with input_image
    if (analyze && typeof analyze.imageUrl === "string" && analyze.imageUrl.trim()) {
      const stream = (await client.responses.stream({
        model: selectedModel,
        instructions: "You are Yurie, a helpful and concise assistant.",
        input: [
          {
            role: "user",
            content: [
              { type: "input_text", text: lastUserMessage || "Analyze this image" },
              {
                type: "input_image",
                image_url: analyze.imageUrl,
                ...(analyze.detail ? { detail: analyze.detail } : {}),
    ...
    const wantsImage =
      Boolean(forceImageTool || (tool_choice && typeof tool_choice === "object" && (tool_choice as { type?: string }).type === "image_generation")) ||
      ((explicitImageVerb.test(lastUserMessage) || imageDescriptorTerms.test(lastUserMessage)) &&
        !analysisIntent.test(lastUserMessage))

    if (wantsImage) {
      try {
        const maybeStreamOrResult = (await client.images.generate({
          model: "gpt-image-1",
          prompt:
            lastUserMessage || "Generate an image based on the conversation context",
          // Image tool options with sensible defaults
          ...
          compression: imageOptions?.compression,
          stream: true,
          partial_images: imageOptions?.partial_images ?? 2,
        } as unknown) as unknown) as unknown

        const encoder = new TextEncoder()
        const mime = imageOptions?.format === "jpeg"
          ? "image/jpeg"
          : imageOptions?.format === "webp"
            ? "image/webp"
            : "image/png"

        const isAsyncIterable = maybeStreamOrResult != null && typeof (maybeStreamOrResult as any)[Symbol.asyncIterator] === "function"

        if (isAsyncIterable) {
          const imgStream = maybeStreamOrResult as AsyncIterable<{ type?: string; b64_json?: string }>
          const readable = new ReadableStream<Uint8Array>({
            async start(controller) {
              ...
              try {
                for await (const event of imgStream) {
                  if (
                    ...
                    if (base64) {
                      controller.enqueue(encoder.encode(`<image:${mime};base64,${base64}>\n`))
                    }
                  }
                }
              } catch (error) {
        ...
        // Fallback for non-streaming clients (e.g., local stub)
        const nonStream = maybeStreamOrResult as { data?: Array<{ b64_json?: string }> }
        const base64 = Array.isArray(nonStream?.data) && nonStream.data[0]?.b64_json
          ? nonStream.data[0].b64_json
          : ""

        const body = `Here is your image.\n${base64 ? `<image:${mime};base64,${base64}>\n` : ""}`
    ...
    const stream = (await client.responses.stream({
      ...
      input: prompt,
      // Include both web search and image_generation tools. Attach image options if provided.
      tools: [
        { type: "web_search_preview" } as { type: string },
        {
          ...
          ...(imageOptions?.format ? { format: imageOptions.format } : {}),
          ...(typeof imageOptions?.compression === "number"
            ? { compression: imageOptions.compression }
            : {}),
